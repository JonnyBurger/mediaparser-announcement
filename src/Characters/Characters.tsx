...
export const Characters: React.FC = () => {
  ...
  const rotXEnd = visualControl("rotateX-end", 1.13, z.number().step(0.01));
  ...
  const scaleStart = visualControl("scale-start", 1.96, z.number().step(0.01));
  const scaleEnd = visualControl("scale-end", 1.96, z.number().step(0.01));
  const transXStart = visualControl("transX-start", -1022, z.number().step(1));
  const transXEnd = visualControl("transX-end", -1022, z.number().step(1));
  const transYStart = visualControl("transY-start", -245, z.number().step(1));
  const transYEnd = visualControl("transY-end", -245, z.number().step(1));
